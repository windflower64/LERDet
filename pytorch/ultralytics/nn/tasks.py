...
def _initialize_yolo_model(model, cfg, ch, nc, verbose):
    ...
    rgbt_silence_root = model.yaml["backbone"][0][2] == "Silence" and int(model.yaml.get("ch", ch)) == 4
    if model.yaml["backbone"][0][2] == "Silence" and not rgbt_silence_root:
        LOGGER.warning(
...
def load_dual_backbone_weights(model, vis_weights=None, ir_weights=None, verbose=True):
    """Load modality-specific backbone weights into an RGBT model.

    The visible and infrared checkpoints are mapped into the two modality branches of the RGBT backbone.
    Layer 9 of the single-modality backbone (SPPF) is mapped to the shared SPPF layer after fusion.
    """
    if vis_weights is None and ir_weights is None:
        return 0

    def _get_model_state(weights):
        src_model = weights["model"] if isinstance(weights, dict) else weights
        return src_model.float().state_dict()

    def _map_branch(src_sd, dst_sd, dst_offset, prefix, load_sppf=False):
        transferred = {}
        for k, v in src_sd.items():
            ...
            if src_idx <= 8:
                dst_key = f"model.{src_idx + dst_offset}.{m.group(2)}"
            elif load_sppf and src_idx == 9:
                dst_key = "model.24." + m.group(2)
            else:
                continue
            if dst_key not in dst_sd:
                continue
            dst_v = dst_sd[dst_key]
            if v.shape != dst_v.shape:
                if src_idx == 0 and k.endswith("conv.weight") and v.ndim == 4 and dst_v.ndim == 4 and v.shape[1] == 3 and dst_v.shape[1] == 1:
                    v = v.mean(dim=1, keepdim=True)
                else:
                    continue
            transferred[dst_key] = v
        if verbose:
            LOGGER.info(f"Prepared {len(transferred)} tensors from {prefix} backbone weights")
        return transferred
    ...
    if vis_weights is not None:
        vis_sd = _get_model_state(vis_weights)
        vis_map = _map_branch(vis_sd, dst_sd, dst_offset=2, prefix="VIS", load_sppf=True)
        for k, v in vis_map.items():
            merged[k] = merged.get(k, 0) + v
            counts[k] = counts.get(k, 0) + 1
    if ir_weights is not None:
        ir_sd = _get_model_state(ir_weights)
        ir_map = _map_branch(ir_sd, dst_sd, dst_offset=12, prefix="IR", load_sppf=True)
    ...
    for k, v in merged.items():
        updated[k] = v / counts[k] if counts[k] > 1 else v
    ...
    if verbose:
        LOGGER.info(
            f"Transferred {len(updated)}/{len(model.state_dict())} items from dual backbone pretrained weights"
        )
    return len(updated)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f], *args]
        elif m in frozenset({TargetAwareGuide, CrossModalGuide}):
            c1, c2 = ch[f[0]], ch[f[1]]
            if c1 != c2:
                raise ValueError(f"{m.__name__} expects same channels, got {c1} and {c2} at layer {i}")
            args = [c1, c1, *args]
            c2 = c1
        elif m is IGSA:
            c1, c_ir, c2 = ch[f[0]], ch[f[1]], args[0]
            if c1 != c_ir:
                raise ValueError(f"IGSA expects same VIS/IR channels, got {c1} and {c_ir} at layer {i}")
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            if c1 != c2:
                raise ValueError(f"IGSA expects output channels to match input channels, got {c1} and {c2} at layer {i}")
            args = [c1, c2, *args[1:]]
