...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        channels: int = 3,
        use_simotm: str = "RGB",
        pairs_rgb_ir: list[str] | tuple[str, str] = ("visible", "infrared"),
    ):
        ...
        self.pairs_rgb_ir = pairs_rgb_ir if len(pairs_rgb_ir) == 2 else ("visible", "infrared")
        self.rgbt = self.channels == 4 and str(self.use_simotm).upper() == "RGBT"
        ...
        self.npy_files = [Path(f).with_suffix(".rgbt.npy" if self.rgbt else ".npy") for f in self.im_files]
    ...
    def paired_image_file(self, im_file: str) -> str:
        """Return paired infrared image path for an RGB image path in an RGBT dataset."""
        rgb_dir, ir_dir = self.pairs_rgb_ir
        paired = im_file.replace(f"{os.sep}{rgb_dir}{os.sep}", f"{os.sep}{ir_dir}{os.sep}")
        if paired == im_file:
            paired = im_file.replace(f"/{rgb_dir}/", f"/{ir_dir}/").replace(f"\\{rgb_dir}\\", f"\\{ir_dir}\\")
        return paired

    def load_rgbt_image_file(self, im_file: str) -> np.ndarray:
        """Load paired RGB and infrared images and concatenate them into a 4-channel array."""
        rgb = imread(im_file, flags=cv2.IMREAD_COLOR)
        ir_file = self.paired_image_file(im_file)
        ir = imread(ir_file, flags=cv2.IMREAD_GRAYSCALE)
        if rgb is None:
            raise FileNotFoundError(f"RGB image not found {im_file}")
        if ir is None:
            raise FileNotFoundError(f"Infrared image not found {ir_file}")
            ...
            if fn.exists():  # load npy
                try:
                    ...
                    if npy_channels != self.channels:
                        ...
                        im = self.load_rgbt_image_file(f) if self.rgbt else imread(f, flags=self.cv2_flag)
                except Exception as e:
                    LOGGER.warning(f"{self.prefix}Removing corrupt *.npy image file {fn} due to: {e}")
                    Path(fn).unlink(missing_ok=True)
                    im = self.load_rgbt_image_file(f) if self.rgbt else imread(f, flags=self.cv2_flag)  # BGR
            else:  # read image
                im = self.load_rgbt_image_file(f) if self.rgbt else imread(f, flags=self.cv2_flag)  # BGR
            if im is None:
    ...
    def cache_images_to_disk(self, i: int) -> None:
        ...
        if not f.exists():
            try:
                im = self.load_rgbt_image_file(self.im_files[i]) if self.rgbt else imread(self.im_files[i], flags=self.cv2_flag)
