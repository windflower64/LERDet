...
def get_label_cache_path(label_files: list[str]) -> Path:
    """Return the label cache path for detection datasets.

    This local RGBT baseline repo defaults to labels_1.cache to avoid sharing
    labels.cache with other projects that may be training on the same dataset.
    """
    cache_path = Path(label_files[0]).parent.with_suffix(".cache")
    cache_name = os.getenv("ULTRALYTICS_LABEL_CACHE_NAME")
    if cache_name:
        return cache_path.with_name(cache_name)

    suffix = os.getenv("ULTRALYTICS_LABEL_CACHE_SUFFIX", "_1")
    if not suffix:
        return cache_path
...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self) -> list[dict]:
        ...
        cache_path = get_label_cache_path(self.label_files)
        try:
