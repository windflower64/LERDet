...
def bbox_iou(
    ...
    if CIoU or DIoU or GIoU or WIoU:
        ...
        if CIoU or DIoU or WIoU:  # Distance, Complete or Wise IoU
            ...
            if WIoU:
                mode = WIoU if isinstance(WIoU, str) else "v1"
                with torch.no_grad():
                    r_w = (-rho2 / c2).exp()
                if str(mode).lower() == "v3":
                    l_iou = (1.0 - iou).clamp(min=eps, max=1.0)
                    l_iou_mean = l_iou.detach().mean().clamp(min=eps)
                    beta = (l_iou.detach() / l_iou_mean).clamp(min=eps, max=6.0)
                    alpha, delta = 1.7, 2.7
                    gain = beta / (delta * (alpha ** (beta - delta)))
                    return iou * r_w * gain.clamp(min=0.2, max=2.0)
