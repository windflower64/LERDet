...
class BboxLoss(nn.Module):
    ...
    def __init__(
        ...
        use_official_wiou: bool = False,
        use_ema_wiseiou: bool = False,
        ema_wiseiou_momentum: float = 0.95,
        use_st_iou: bool = False,
        st_iou_lambda: float = 0.25,
        st_iou_tau: float = 16.0,
        st_iou_kappa: float = 4.0,
    ):
        """Initialize bounding-box losses, including the optional small-target ST-IoU loss."""
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.use_wiseiou = bool(use_wiseiou)
        self.use_official_wiou = bool(use_official_wiou)
        self.use_ema_wiseiou = bool(use_ema_wiseiou)
        self.ema_wiseiou_momentum = float(ema_wiseiou_momentum)
        self.use_st_iou = bool(use_st_iou)
        self.st_iou_lambda = float(st_iou_lambda)
        self.st_iou_tau = float(st_iou_tau)
        self.st_iou_kappa = float(st_iou_kappa)
        self.register_buffer("ema_wiseiou_mean", torch.tensor(1.0))
        self.register_buffer("ema_wiseiou_initialized", torch.tensor(False))
        self.register_buffer("official_wiou_mean", torch.tensor(1.0))
        self.official_wiou_momentum = 1e-2
        self.official_wiou_alpha = 1.7
        self.official_wiou_delta = 2.7
        wiou_mode_count = sum(
            (
                int(self.use_wiseiou),
                int(self.use_official_wiou),
                int(self.use_ema_wiseiou),
                int(self.use_st_iou),
            )
        )
        if wiou_mode_count > 1:
            raise ValueError("WIoU/ST-IoU modes are mutually exclusive. Enable only one of them at a time.")
        if not 0.0 <= self.ema_wiseiou_momentum < 1.0:
            raise ValueError("EMA-WIoU momentum must be in [0, 1).")
        if min(self.st_iou_lambda, self.st_iou_tau, self.st_iou_kappa) < 0:
            raise ValueError("ST-IoU hyperparameters must be non-negative.")
        if self.use_st_iou and min(self.st_iou_tau, self.st_iou_kappa) == 0:
            raise ValueError("ST-IoU tau and kappa must be positive.")

    @staticmethod
    def _dynamic_quality_factor(
        pred_bboxes: torch.Tensor,
        target_bboxes: torch.Tensor,
        iou: torch.Tensor,
        mean_iou_loss: torch.Tensor | None = None,
        eps: float = 1e-7,
    ) -> torch.Tensor:
        """Return the detached distance and non-monotonic quality factor used by the local IoU baseline."""
        ...
        convex_diag_sq = convex_w.square() + convex_h.square() + eps
        center_dist_sq = (
            (t_x1 + t_x2 - p_x1 - p_x2).square() + (t_y1 + t_y2 - p_y1 - p_y2).square()
        ) / 4

        with torch.no_grad():
            distance_factor = (-center_dist_sq / convex_diag_sq).exp()
            iou_loss = (1.0 - iou).clamp(min=eps, max=1.0)
            if mean_iou_loss is None:
                mean_iou_loss = iou_loss.mean().clamp(min=eps)
            else:
                mean_iou_loss = mean_iou_loss.to(device=iou.device, dtype=iou.dtype).clamp(min=eps)
            beta = (iou_loss / mean_iou_loss).clamp(min=eps, max=6.0)
            alpha, delta = 1.7, 2.7
            focusing_gain = beta / (delta * (alpha ** (beta - delta)))
            return distance_factor * focusing_gain.clamp(min=0.2, max=2.0)

    def _ema_iou_loss_mean(self, iou: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
        """Update and return the EMA reference IoU loss for EMA-stabilized WIoU."""
        with torch.no_grad():
            batch_mean = (1.0 - iou.detach()).clamp(min=eps, max=1.0).mean().clamp(min=eps)
            if self.training:
                if not bool(self.ema_wiseiou_initialized.item()):
                    self.ema_wiseiou_mean.copy_(batch_mean)
                    self.ema_wiseiou_initialized.fill_(True)
                else:
                    self.ema_wiseiou_mean.mul_(self.ema_wiseiou_momentum).add_(
                        batch_mean * (1.0 - self.ema_wiseiou_momentum)
                    )
            return self.ema_wiseiou_mean.detach().clone().clamp(min=eps)

    def _small_target_scale_weight(
        self,
        target_bboxes: torch.Tensor,
        foreground_stride: torch.Tensor,
        sample_weight: torch.Tensor,
        eps: float = 1e-7,
    ) -> torch.Tensor:
        """Return assignment-weight-normalized small-target weights for ST-IoU."""
        target_pixels = target_bboxes * foreground_stride
        t_x1, t_y1, t_x2, t_y2 = target_pixels.chunk(4, -1)
        target_scale = ((t_x2 - t_x1).clamp(min=0) * (t_y2 - t_y1).clamp(min=0)).clamp(min=1e-7).sqrt()
        scale_gate = torch.sigmoid((self.st_iou_tau - target_scale) / self.st_iou_kappa)
        raw_weight = 1.0 + self.st_iou_lambda * scale_gate
        with torch.no_grad():
            normalizer = (raw_weight * sample_weight).sum() / sample_weight.sum().clamp(min=eps)
        return raw_weight / normalizer.clamp(min=eps)

    def _official_wiou_loss(
        self,
        pred_bboxes: torch.Tensor,
        target_bboxes: torch.Tensor,
        eps: float = 1e-7,
    ) -> torch.Tensor:
        """Return the official Wise-IoU v3-style per-box loss from the public reference implementation."""
        pred_xy = (pred_bboxes[..., :2] + pred_bboxes[..., 2:4]) / 2
        target_xy = (target_bboxes[..., :2] + target_bboxes[..., 2:4]) / 2
        pred_wh = (pred_bboxes[..., 2:4] - pred_bboxes[..., :2]).clamp(min=0.0)
        target_wh = (target_bboxes[..., 2:4] - target_bboxes[..., :2]).clamp(min=0.0)

        mins = torch.minimum(pred_bboxes, target_bboxes)
        maxs = torch.maximum(pred_bboxes, target_bboxes)
        inter_wh = (mins[..., 2:4] - maxs[..., :2]).clamp(min=0.0)
        inter = inter_wh[..., 0] * inter_wh[..., 1]
        union = pred_wh[..., 0] * pred_wh[..., 1] + target_wh[..., 0] * target_wh[..., 1] - inter + eps
        iou_loss = (1.0 - inter / union).clamp(min=eps, max=2.0)

        center_dist_sq = (pred_xy - target_xy).square().sum(-1)
        enclosing_wh = (maxs[..., 2:4] - mins[..., :2]).clamp(min=0.0)
        enclosing_diag_sq = enclosing_wh.square().sum(-1).clamp(min=eps)

        if self.training:
            with torch.no_grad():
                batch_mean = iou_loss.detach().mean().clamp(min=eps)
                self.official_wiou_mean.mul_(1.0 - self.official_wiou_momentum).add_(
                    batch_mean * self.official_wiou_momentum
                )

        dist_term = torch.exp(center_dist_sq / enclosing_diag_sq.detach())
        base_loss = dist_term * iou_loss
        beta = (iou_loss.detach() / self.official_wiou_mean.clamp(min=eps)).clamp(min=eps)
        divisor = self.official_wiou_delta * (self.official_wiou_alpha ** (beta - self.official_wiou_delta))
        return (base_loss * (beta / divisor)).unsqueeze(-1)
    ...
    def forward(
        ...
        if self.use_official_wiou:
            per_box_loss = self._official_wiou_loss(foreground_pred, foreground_target)
        elif self.use_ema_wiseiou:
            iou = bbox_iou(foreground_pred, foreground_target, xywh=False)
            mean_iou_loss = self._ema_iou_loss_mean(iou)
            quality_factor = self._dynamic_quality_factor(foreground_pred, foreground_target, iou, mean_iou_loss)
            per_box_loss = 1.0 - iou * quality_factor
        elif self.use_st_iou:
            iou = bbox_iou(foreground_pred, foreground_target, xywh=False)
            quality_factor = self._dynamic_quality_factor(foreground_pred, foreground_target, iou)
            base_score = iou * quality_factor
            expanded_stride = stride.view(1, -1, 1).expand(*target_bboxes.shape[:2], 1)
            foreground_stride = expanded_stride[fg_mask]
            scale_weight = self._small_target_scale_weight(
                foreground_target,
                foreground_stride,
                weight,
            )
            per_box_loss = scale_weight * (1.0 - base_score)
        else:
            iou = bbox_iou(
                foreground_pred,
                foreground_target,
                xywh=False,
                WIoU="v3" if self.use_wiseiou else False,
                CIoU=not self.use_wiseiou,
            )
            per_box_loss = 1.0 - iou
        loss_iou = (per_box_loss * weight).sum() / target_scores_sum
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        get_hyp = h.get if isinstance(h, dict) else lambda key, default: getattr(h, key, default)
        self.bbox_loss = BboxLoss(
            m.reg_max,
            use_wiseiou=get_hyp("use_wiseiou", False),
            use_official_wiou=get_hyp("use_official_wiou", False),
            use_ema_wiseiou=get_hyp("use_ema_wiseiou", False),
            ema_wiseiou_momentum=get_hyp("ema_wiseiou_momentum", 0.95),
            use_st_iou=get_hyp("use_st_iou", False),
            st_iou_lambda=get_hyp("st_iou_lambda", 0.25),
            st_iou_tau=get_hyp("st_iou_tau", 16.0),
            st_iou_kappa=get_hyp("st_iou_kappa", 4.0),
        ).to(device)
