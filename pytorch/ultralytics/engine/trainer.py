...
from ultralytics.nn.tasks import load_checkpoint, load_dual_backbone_weights
...
class BaseTrainer:
    ...
    def setup_model(self):
        ...
        vis_weights = getattr(self.args, "pretrained_vis", None)
        ir_weights = getattr(self.args, "pretrained_ir", None)
        if vis_weights or ir_weights:
            vis_ckpt = load_checkpoint(vis_weights)[0] if vis_weights else None
            ir_ckpt = load_checkpoint(ir_weights)[0] if ir_weights else None
            load_dual_backbone_weights(self.model, vis_weights=vis_ckpt, ir_weights=ir_ckpt, verbose=RANK in {-1, 0})
        return ckpt
